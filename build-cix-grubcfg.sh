...
do_build() {
    ...
    if [[ "${ACPI}" == "0" ]]; then
        ...
        replace_or_add_line "GRUB_CMDLINE_LINUX=" "GRUB_CMDLINE_LINUX=\"console=ttyAMA0,115200 efi=noruntime earlycon=pl011,0x040d0000 loglevel=4 arm-smmu-v3.disable_bypass=0 acpi=off pcie_aspm=off\"" "$PATH_OUT_DEB_PACKAGES/cix-grubcfg/etc/default/grub.d/cix_grub.cfg"
     else
        replace_or_add_line "devicetree" "\ " "$PATH_OUT_DEB_PACKAGES/cix-grubcfg/etc/grub.d/09_cix_linux"
        replace_or_add_line "GRUB_CMDLINE_LINUX=" "GRUB_CMDLINE_LINUX=\"console=ttyAMA0,115200 efi=noruntime earlycon=pl011,0x040d0000 loglevel=4 arm-smmu-v3.disable_bypass=0 cma=640M acpi=force pcie_aspm=off\"" "$PATH_OUT_DEB_PACKAGES/cix-grubcfg/etc/default/grub.d/cix_grub.cfg"
    fi
